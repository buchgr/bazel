...
public class GrpcActionCache implements RemoteActionCache {
  ...
  private final GrpcByteStreamInterface bsIface;
  private final GrpcActionCacheInterface acIface;
  ...
  public GrpcActionCache(
      ...
      GrpcActionCacheInterface acIface) {
    this.options = options;
    this.casIface = casIface;
    this.bsIface = bsIface;
    this.acIface = acIface;
  }

  @VisibleForTesting
  public GrpcActionCache(Channel channel, RemoteOptions options, ChannelOptions channelOptions) {
    this.options = options;
    this.casIface =
    ...
    this.acIface =
        GrpcInterfaces.actionCacheInterface(options.remoteTimeout, channel, channelOptions);
  }
  ...
  private ImmutableSet<Digest> getMissingDigests(Iterable<Digest> digests) {
    ...
    FindMissingBlobsResponse response = casIface.findMissingBlobs(request.build());
  ...
  public void uploadTree(TreeNodeRepository repository, Path execRoot, TreeNode root)
    ...
    if (!treeNodes.isEmpty()) {
      ...
      BatchUpdateBlobsResponse response = casIface.batchUpdateBlobs(treeBlobRequest.build());
  ...
  private Iterator<ReadResponse> readBlob(Digest digest) throws CacheNotFoundException {
    ...
    try {
      return bsIface.read(ReadRequest.newBuilder().setResourceName(resourceName).build());
  ...
  static class WriteResponseStreamObserver implements StreamObserver<WriteResponse> {
    private final CountDownLatch finishLatch;
    private final AtomicReference<RuntimeException> exception;
    private long bytesLeft;

    public WriteResponseStreamObserver(
        long totalSize, CountDownLatch finishLatch, AtomicReference<RuntimeException> exception) {
      this.finishLatch = finishLatch;
      this.exception = exception;
      this.bytesLeft = totalSize;
    }
    ...
    public void onCompleted() {
      if (bytesLeft != 0) {
        this.exception.compareAndSet(null, new RuntimeException("Server did not commit all data."));
      }
      finishLatch.countDown();
    }
  }
  ...
  private void uploadChunks(int numItems, Chunker chunker)
    ...
    while (chunker.hasNext()) {
      ...
      if (offset == 0) { // Beginning of new upload.
        ...
        requestObserver =
            bsIface.write(
                new WriteResponseStreamObserver(digest.getSizeBytes(), finishLatch, exception));
      }
  ...
  public ActionResult getCachedActionResult(ActionKey actionKey) {
    try {
      return acIface.getActionResult(
