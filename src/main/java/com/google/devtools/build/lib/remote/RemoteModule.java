...
public final class RemoteModule extends BlazeModule {
  ...
  public void beforeCommand(CommandEnvironment env) throws AbruptExitException {
    ...
    try {
      ...
      RemoteCacheClient cacheProtocol = null;
      ...
      if (enableBlobStoreCache) {
        executeRetrier = null;
        cacheProtocol = SimpleBlobStoreFactory.create(
            remoteOptions,
      ...
      if (enableRemoteExecution) {
        ...
        Preconditions.checkState(
            cacheProtocol instanceof GrpcCacheClient,
            "Only the gRPC cache is support for remote execution");
        RemoteExecutionCache remoteCache =
            new RemoteExecutionCache((GrpcCacheClient) cacheProtocol, remoteOptions, digestUtil);
        actionContextProvider =
            RemoteActionContextProvider.createForRemoteExecution(
                env, remoteCache, executor, executeRetrier, digestUtil, logDir);
      } else if (cacheProtocol != null) {
        RemoteCache remoteCache = new RemoteCache(cacheProtocol, remoteOptions, digestUtil);
        actionContextProvider =
