...
abstract class AbstractRemoteActionCache implements AutoCloseable {
  ...
  static class ActionOutputMetadata {
    ...
    private final ImmutableMap<Path, OutputFileMetadata> outputFilesMetadata;
    private final ImmutableMap<Path, OutputSymlinkMetadata> outputSymlinksMetadata;
    private final ImmutableMap<Path, ImmutableList<OutputFileMetadata>> outputDirectoriesMetadata;

    public ActionOutputMetadata(
        ImmutableMap<Path, OutputFileMetadata> outputFilesMetadata,
        ImmutableMap<Path, OutputSymlinkMetadata> outputSymlinksMetadata,
        ImmutableMap<Path, ImmutableList<OutputFileMetadata>> outputDirectoriesMetadata) {
      this.outputFilesMetadata = outputFilesMetadata;
      this.outputSymlinksMetadata = outputSymlinksMetadata;
      this.outputDirectoriesMetadata = outputDirectoriesMetadata;
    }

    @Nullable
    public OutputFileMetadata fileMetadata(Path path) {
      return outputFilesMetadata.get(path);
    }

    @Nullable
    public OutputSymlinkMetadata outputSymlinkMetadata(Path path) {
      return outputSymlinksMetadata.get(path);
    }

    @Nullable
    public ImmutableList<OutputFileMetadata> treeArtifactMetadata(Path path) {
      return outputDirectoriesMetadata.get(path);
    }

    public Collection<OutputFileMetadata> outputFilesMetadata() {
      return outputFilesMetadata.values();
    }

    public Collection<ImmutableList<OutputFileMetadata>> outputDirectoriesMetadata() {
      return outputDirectoriesMetadata.values();
    }

    public Collection<OutputSymlinkMetadata> outputSymlinksMetadata() {
      return outputSymlinksMetadata.values();
    }
  }

  // TODO(buchgr): This could end up in a infinite recursion.
  public ImmutableList<OutputFileMetadata> computeFilesInDirectory(
      Path path, Directory dir, Map<Digest, Directory> childDirectoriesMap) throws IOException {
    ImmutableList.Builder<OutputFileMetadata> filesBuilder = ImmutableList.builder();
    if (dir.getSymlinksCount() > 0) {
      throw new IOException("symlinks are not yet supported");
    }
  ...
  private ActionOutputMetadata retrieveOutputMetadata(ActionResult actionResult, Path execRoot)
    ...
    for (Map.Entry<Path, ListenableFuture<Tree>> metadataDownload :
      ...
      Tree directoryTree = Utils.getFromFuture(metadataDownload.getValue());
  ...
  public void download(ActionResult result, Path execRoot, FileOutErr outErr)
    ...
    List<ListenableFuture<OutputFileMetadata>> downloads =
        Stream.concat(
                metadata.outputFilesMetadata().stream(),
                metadata.outputDirectoriesMetadata().stream().flatMap(Collection::stream))
            .map(
                (file) -> {
  ...
  public void downloadStdoutStderr(ActionResult result, FileOutErr outErr) throws IOException, InterruptedException {
